...
namespace XLanceDLL
{
	...
	public partial class TreeViewColumns : UserControl
	{
        ...
        private void treeView1_DragDrop(object sender, System.Windows.Forms.DragEventArgs e)
        {
            ...
            TreeNode tempNode = (TreeNode)tn.Clone();
            int iFileType = GetItemType(tempNode);

            TreeNode ParentNode = treeView1.GetNodeAt(pt);

            int iItemParenType = GetItemType(ParentNode);

            if ((iFileType == (int)FileType.PAGE && iItemParenType != (int)FileType.FILETYPE) ||
                (iFileType == (int)FileType.FILETYPE && iItemParenType != (int)FileType.FOLDER) ||
                (iFileType == (int)FileType.PAGE && iItemParenType == (int)FileType.FOLDER) ||
                (iFileType == (int)FileType.FOLDER && iItemParenType == (int)FileType.PAGE) ||
                (iFileType == (int)FileType.FILETYPE && iItemParenType == (int)FileType.PAGE) ||
                (iFileType == (int)FileType.FOLDER && iItemParenType == (int)FileType.FILETYPE) ||
                (iFileType == (int)FileType.NOTYPE || iItemParenType == (int)FileType.NOTYPE))
            {
                MessageBox.Show("Operation not allowed", "Stop", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            else if (iFileType == (int)FileType.PAGE && iItemParenType == (int)FileType.PAGE)
            {
                ParentNode = ParentNode.Parent;
            }
            ParentNode.Nodes.Add(tempNode); // this copies the node
            tn.Remove(); // need to remove the original version of the node
        }

        int GetItemType(TreeNode treeNode)
        {
            string[] itemString = (string[])treeNode.Tag;
            string str = itemString.ElementAt(8);
            if(str.Length == 0)
                return (int)FileType.NOTYPE;

            if (Directory.Exists(str))
            {
                return (int)FileType.FOLDER;
            }
            else if (File.Exists(str))
            {
                return (int)FileType.FILETYPE;
            }
            return (int)FileType.PAGE;
        }
