...
{ 
  ...
  PostMessage(esd->Parent, WM_SIZE,SIZE_RESTORED,
    MAKELPARAM(pr.bottom,pr.right)); 
}
...
{ 
  ...
  { 
    ...
    ppen = SelectObject(dc,pen);
    Rectangle(dc,cr.left,cr.top,cr.right,cr.bottom);
    SelectObject(dc,ppen);
    DeleteObject(pen); 
  }
  else
    FillRect(dc,&cr,GetCurrentObject(dc,OBJ_BRUSH));  
