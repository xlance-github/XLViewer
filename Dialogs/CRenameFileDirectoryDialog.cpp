...
// Purpose:   CRenameFileDirectoryDialog object implementation
//
//
//
// Date:      
...
// Module Level Vars
static CRenameFileDirectoryDialog *pcrfddlgThis = NULL;

/**
 * Default constructor, initializes all fields to their defaults.
 */
CRenameFileDirectoryDialog::CRenameFileDirectoryDialog()
{
	...
	m_strNewObjectName = EMPTY_STRING;
 ...
 *
 */
CRenameFileDirectoryDialog::CRenameFileDirectoryDialog(HINSTANCE hInstance,
	TCHAR *tstrFilename)
{
	try
	{
		// initialize fields 
		m_strNewObjectName = tstrFilename;
		m_strLastError = EMPTY_STRING;
		m_hbrBackground = NULL;
		m_hfontControls = NULL;
	...
	catch(...)
	{
		...
		// set last error
		m_strLastError = _T("While intializing the rename file/directory dialog object, an unspecified exception occurred.");
	}
}
 ...
 */
LRESULT CALLBACK CRenameFileDirectoryDialog::WindowProc (HWND hwnd, UINT uMsg, 
	WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
			// set handle
			pcrfddlgThis->m_hwndThis = hwnd;

			// load this dialog's preferences
			pcrfddlgThis->loadPreferences();
			
			// set icon
			pcrfddlgThis->loadIcon();

			// set font for controls
			//pcrfddlgThis->setControlFonts();

			// Register escape key
			RegisterHotKey(hwnd, HOTKEY_ESCAPE, (UINT)0, VK_ESCAPE);

			// set initial name for object to its current one
			SetDlgItemText(hwnd, IDC_TXTOBJECTNAME, 
				(TCHAR *)pcrfddlgThis->m_strNewObjectName.data());
			break;			
		...
		//	{
		//		// create black brush
		//		if(pcrfddlgThis->m_hbrBackground == NULL)
		//			pcrfddlgThis->m_hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);
		...
		//		
		//		// return brush
		//		return (LRESULT)pcrfddlgThis->m_hbrBackground;
		//	}
		//	break;

		case WM_DESTROY:
			// save preferences for this dialog ALWAYS
			pcrfddlgThis->savePreferences();

			// clear handle because window is being closed manually.
			pcrfddlgThis->m_hwndThis = NULL;
 ...
 */
int CRenameFileDirectoryDialog::show()
{
    ...
    try
    {
		...
		iReturn = DialogBox(m_hinstApplication, 
							MAKEINTRESOURCE(IDD_DLGRENAMEFILEDIRECTORY),
 ...
 *
 */
BOOL CRenameFileDirectoryDialog::loadPreferences()
{
    ...
    try
    {
		...
		GetRegistryBinary(CurrentUser, REG_BASE, 
			...
			REG_VAL_PREFS_LOCATION_RENAMEOBJECT, &rctThis, 
 ...
 *
 */
BOOL CRenameFileDirectoryDialog::savePreferences()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
 ...
 *
 */
BOOL CRenameFileDirectoryDialog::loadIcon()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
 ...
 * @return
 */
BOOL CRenameFileDirectoryDialog::setControlFonts()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
 ...
 */
BOOL CRenameFileDirectoryDialog::saveObjectName()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
    ...
    try
    {
		...
		GetDlgItemText(m_hwndThis, IDC_TXTOBJECTNAME, tstrBuffer, 
			sizeof(tstrBuffer));

		// store in member field
		m_strNewObjectName = tstrBuffer;

		// check returned directory name
		if(m_strNewObjectName.length() == 0)
			bReturn = FALSE;
    }
    catch(...)
    {
        // set last error
        m_strLastError = _T("While saving the new object name, an unexpected error occurred.");
