...
// Purpose:   CHelpDialog object implementation
//
//
//
// Date:      
...
// Module Level Vars
static CHelpDialog *pchlpdlgThis = NULL;
 ...
 *
 */
CHelpDialog::CHelpDialog(HINSTANCE hInstance)
{
	try
	{
		// initialize fields 
 ...
 */
LRESULT CALLBACK CHelpDialog::WindowProc (HWND hwnd, UINT uMsg, 
	WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
			// set handle
			pchlpdlgThis->m_hwndThis = hwnd;

			// load this dialog's preferences
			pchlpdlgThis->loadPreferences();

			// set icon
			pchlpdlgThis->loadIcon();

			// set font for controls
			pchlpdlgThis->setControlFonts();

			// load help text
			pchlpdlgThis->loadHelpText();
		...
		//	{
		//		// create black brush
		//		if(pchlpdlgThis->m_hbrBackground == NULL)
		//			pchlpdlgThis->m_hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);
		...
		//		
		//		// return brush
		//		return (LRESULT)pchlpdlgThis->m_hbrBackground;
		//	}
		//	break;

		case WM_DESTROY:
			// save preferences for this dialog ALWAYS
			pchlpdlgThis->savePreferences();

			// clear handle because window is being closed manually.
			pchlpdlgThis->m_hwndThis = NULL;
		...
		case WM_COMMAND:
			...
			{
				case IDC_CMDABOUT:
					// unregister ESC key so it can be used by about dialog
					UnregisterHotKey(hwnd, HOTKEY_ESCAPE);

					// display about dialog
					pchlpdlgThis->displayAbout();
		...
		case WM_SHOWWINDOW:
			...
			hwndTemp = GetDlgItem(hwnd, IDC_TXTHELP);
 ...
 *
 */
BOOL CHelpDialog::loadPreferences()
{
    ...
    try
    {
		...
		GetRegistryBinary(CurrentUser, REG_BASE, 
			...
			REG_VAL_PREFS_LOCATION_HELP, &rctThis, 
 ...
 *
 */
BOOL CHelpDialog::savePreferences()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
 ...
 * @return
 */
BOOL CHelpDialog::setControlFonts()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
    ...
    try
    {
		...
		hwndTemp = GetDlgItem(m_hwndThis, IDC_TXTHELP);
		if(hwndTemp == NULL)
			return FALSE;
...
BOOL CHelpDialog::loadHelpText()
{
    ...
    try
    {
		...
		hresText = FindResource(NULL, MAKEINTRESOURCE(IDR_HELPTEXT), 
						RESOURCE_TYPE_FILES);
		if(hresText == NULL)
			return FALSE;

		// get actual size of text
 ...
 * @return TRUE if no errors occur, otherwise FALSE.
 */
BOOL CHelpDialog::displayAbout()
{
	CAboutDialog *pcabtdlgThis = NULL;
	BOOL bReturn = TRUE;	// default to optimistic return val
    ...
    try
    {
		...
		pcabtdlgThis = new CAboutDialog(m_hinstApplication);

		// validate, continue
		if(pcabtdlgThis == NULL)
		{
			// set last error
