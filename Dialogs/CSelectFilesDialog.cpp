...
// Purpose:   CSelectFilesDialog object implementation
//
//
//
// Date:      
...
// Module Level Vars
static CSelectFilesDialog *pcsfdlgThis = NULL;

/**
 * Default constructor, initializes all fields to their defaults.
 */
CSelectFilesDialog::CSelectFilesDialog()
{
 ...
 *
 */
CSelectFilesDialog::CSelectFilesDialog(HINSTANCE hInstance)
{
	try
	{
		// initialize fields 
 ...
 */
LRESULT CALLBACK CSelectFilesDialog::WindowProc (HWND hwnd, UINT uMsg, 
	WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
			// set handle
			pcsfdlgThis->m_hwndThis = hwnd;

			// load this dialog's preferences
			pcsfdlgThis->loadPreferences();
			
			// set icon
			pcsfdlgThis->loadIcon();

			// set font for controls
			//pcsfdlgThis->setControlFonts();
		...
		//	{
		//		// create black brush
		//		if(pcsfdlgThis->m_hbrBackground == NULL)
		//			pcsfdlgThis->m_hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);
		...
		//		
		//		// return brush
		//		return (LRESULT)pcsfdlgThis->m_hbrBackground;
		//	}
		//	break;

		case WM_DESTROY:
			// save preferences for this dialog ALWAYS
			pcsfdlgThis->savePreferences();

			// clear handle because window is being closed manually.
			pcsfdlgThis->m_hwndThis = NULL;
...
int CSelectFilesDialog::show()
{
    ...
    try
    {
		...
		iReturn = DialogBox(m_hinstApplication, 
							MAKEINTRESOURCE(IDD_DLGSELECTFILESBYMASK),
 ...
 *
 */
BOOL CSelectFilesDialog::loadPreferences()
{
    ...
    try
    {
		...
		GetRegistryBinary(CurrentUser, REG_BASE, 
			...
			REG_VAL_PREFS_LOCATION_SELECTFILES, &rctThis, 
 ...
 *
 */
BOOL CSelectFilesDialog::savePreferences()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
 ...
 *
 */
BOOL CSelectFilesDialog::loadIcon()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
 ...
 * @return
 */
BOOL CSelectFilesDialog::setControlFonts()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
 ...
 */
BOOL CSelectFilesDialog::saveFileMask()
{
    BOOL bReturn = TRUE;	// default to optimistic return val
    ...
    try
    {
		...
		GetDlgItemText(m_hwndThis, IDC_TXTFILEMASK, tstrBuffer,  sizeof(tstrBuffer));

		// store in member field
		m_strFileMask = tstrBuffer;
    ...
    {
        // set last error
        m_strLastError = _T("While saving the file mask, an unexpected error occurred.");
